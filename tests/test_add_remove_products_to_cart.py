...
class TestShopPage:
    ...
    def test_items_price_matching_with_total_on_cart_page(self, shop_item_details, navigate_to_cart_page):
        total_amount_on_cart = sum(item['price'] * item['quantity'] for item in shop_item_details["item_details_on_cart_page"].values())
    ...
    def test_items_name_matching_with_total_on_cart_page(self, shop_item_details, navigate_to_cart_page):
        ...
        assert sorted(set(item_names_on_cart_page)) == sorted(set(selected_item_names_from_shop_page))
    ...
    def test_update_items_quantity_in_cart(self, shop_item_details, navigate_to_cart_page):
        ...
        shop_item_details["selected_items_on_shop_page"] = updated_item_quantity

    @pytest.mark.positive_cases
    def test_total_price_after_updating_the_cart(self, shop_item_details):
        expected_amount = sum(item['price'] * item['quantity'] for item in shop_item_details["selected_items_on_shop_page"].values())
        actual_amount_on_cart = self.shop_page_obj.get_total_amount_of_cart()
        assert expected_amount == actual_amount_on_cart